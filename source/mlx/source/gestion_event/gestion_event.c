...
** Last update Tue Jun  4 13:54:39 2013 florian faisant
*/
...
static int	other_key(int keycode, t_data *data)
{
  if (keycode == EXIT)
    ...
    {
      data->scene.eyes.pos.x += PRECISION_MOVE;
      preview_move(data);
    }
    ...
    {
      data->scene.eyes.pos.y += PRECISION_MOVE;
      preview_move(data);
    }
  return (SUCCES);
}
...
static int	gestion_key(int keycode, t_data *data)
{
  ...
  if (keycode == RIGHT)
    ...
    {
      data->scene.eyes.pos.z -= PRECISION_MOVE;
      preview_move(data);
    }
